...
def test_record_reading_and_query() -> None:  # ← Añadido -> None
    ...
    reading = rs.record_reading(user_id=user.id, lectura_actual=150.0, lectura_anterior=100.0)
    assert reading is not None
    assert abs(reading.consumo - 50.0) < 1e-6

    items = rs.get_readings_for_user(user.id)
    assert len(items) >= 1
    assert abs(items[0].consumo - 50.0) < 1e-6


def test_invalid_reading_raises() -> None:  # ← Añadido -> None
    conn = sqlite3.connect(":memory:")
    user_repo = SQLiteUserRepository(connection=conn)
    user_repo.create_tables()
    reading_repo = SQLiteReadingRepository(connection=conn)
    ...
    rs = ReadingService(reading_repo=reading_repo)
    try:
        rs.record_reading(user_id=user.id, lectura_actual=90.0, lectura_anterior=100.0)
