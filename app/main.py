...
def main() -> None:  # ← Añadido "-> None"
    parser = argparse.ArgumentParser(prog="electric_app", description="Gestor de lecturas eléctricas - Fase 1")
    parser.add_argument("command", choices=["init-db", "create-admin", "help"], nargs="?", default="help")
    parser.add_argument("--username", "-u", help="Username del admin a crear")
    parser.add_argument("--password", "-p", help="Password del admin a crear (inseguro en CLI)")
    ...
    if args.command == "init-db":
        ...
        try:
            user = auth.create_user(name="Administrador", username=username, password=password, role="admin")
            ActivityLogger().log_event(user.id, "create_admin", f"Admin {username} creado")
        ...
        print("  python -m app.main create-admin    -> crear usuario admin (pide username/password)")
